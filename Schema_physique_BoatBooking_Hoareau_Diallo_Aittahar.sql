...
CREATE TABLE BATEAU_TYPE (
    ...
    BTYPE_NOM VARCHAR(40) NOT NULL CONSTRAINT CKC_nom CHECK(BTYPE_NOM = UPPER(SUBSTR(BTYPE_NOM, 1, 1)) || SUBSTR(BTYPE_NOM, 2, LENGTH(BTYPE_NOM))),
    BTYPE_PRIX_HEURE NUMBER(6) NOT NULL constraint ckc_btype_prix CHECK (BTYPE_PRIX_HEURE > 0),
...
CREATE TABLE MARQUE (
    ...
    MRQ_NOM VARCHAR(50) NOT NULL CONSTRAINT CKC_nom CHECK(MRQ_NOM = UPPER(SUBSTR(MRQ_NOM, 1, 1)) || SUBSTR(MRQ_NOM, 2, LENGTH(MRQ_NOM))),
    MRQ_FABRICANT VARCHAR(50) NOT NULL,
    MRQ_DESC VARCHAR(255),
