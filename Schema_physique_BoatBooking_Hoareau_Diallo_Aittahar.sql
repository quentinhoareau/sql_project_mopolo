...
CREATE TABLE CLIENT (
    ...
    CL_ADDR_CP VARCHAR(5) NOT NULL constraint CKC_CL_ADDR_CP_SIZE_CLIENT CHECK (length(CL_ADDR_CP) > 4), constraint CKC_CL_ADDR_CP_ONLY_NUMBER_CLIENT CHECK (REGEXP_LIKE(CL_ADDR_CP, '^[[:digit:]]{9}$')),
    ...
    CL_TELEPHONE VARCHAR(10) NOT NULL UNIQUE constraint CKC_CL_TELEPHONE_LENGTH_CLIENT CHECK (length(CL_TELEPHONE) > 9),constraint CKC_CL_TELEPHONE_ONLY_NUMBER_CLIENT CHECK (REGEXP_LIKE(CL_TELEPHONE, '^[[:digit:]]{9}$')),
