...
CREATE TABLE MARQUE (
    ...
    MRQ_COEFF_MAJORATION NUMBER(6,4) NOT NULL constraint CKC_MRQ_COEFF_MAJORAT_MARQUE CHECK (
        MRQ_COEFF_MAJORATION BETWEEN 1
        AND 1.9999
    ),
...
CREATE TABLE RESERVATION (
    ...
    RES_DATE_DEBUT TIMESTAMP NOT NULL,
    RES_DATE_FIN TIMESTAMP NOT NULL,
    RES_NOTE VARCHAR(300),
    constraint PK_RESERVATION primary key (CL_ID, BT_IMMATRICULE,RES_DATE_DEBUT),
    constraint FK_RESERVATION_BATEAU foreign key (BT_IMMATRICULE) references BATEAU (BT_IMMATRICULE),
    constraint FK_RESERVATION_CLIENTELE foreign key (CL_ID) references CLIENTELE (CL_ID),
    constraint UNIQUE_IMMA_DATEDEB UNIQUE (BT_IMMATRICULE,RES_DATE_DEBUT)
);
